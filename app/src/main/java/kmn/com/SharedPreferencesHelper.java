...
public class SharedPreferencesHelper {
    ...
    public User login(String login, String passsword) {
        List<User> users = getUsers();
        for (User u : users) {
            if (login.equalsIgnoreCase(u.getmLogin())
                    && passsword.equals(u.getmPassword())) {
                u.setHasSuccessLogin(true);
                mSharedPreferences.edit().putString(USERS_KEY, mGson.toJson(users, USERS_TYPE)).apply();
                return u; // возвращаем userа еслинайдем
            }
        }
        return null; // если не найдем то возвращаем null
    }
